...
procedure pr326();
begin
  {Построить траекторию тела,брошенного под углом а=30 и 
    начальной скоростью 50 м/с (без учёта силы сопротивления). Что это
    за траектория? Подтвердите аналитически. }
    Init(1, 250, 250, 0);
    var body_x, body_y, angle, velocity, velocity_x, velocity_y,
    acceleration_x, acceleration_y, dtime: real;
    body_y := 0.1;
    body_x := 0;
    velocity := 50;
    angle := 30;
    
    velocity_x := velocity * cos(DegToRad(angle));
    velocity_y := velocity * sin(DegToRad(angle));
    ...
    dtime := 0.001;
    Pen.Width := 100;
    OutRes(body_x, body_y, colors.Red);
    
    while body_y > 0 do
    begin
        body_x := body_x + velocity_x * dtime;
        body_y := body_y + velocity_y * dtime;
        
        velocity_x := velocity_x + acceleration_x * dtime;
        velocity_y := velocity_y + acceleration_y * dtime;
...
procedure pr327();
begin
  {Добавить в программу строки,выводящие на экран угол броска и дальность полета
    Варьируя угол бросания(не меняя нач скорость) найти такое его значение
    при котором достигается максимальная дальность полета
    Чему равна равна максимальная дальность полета?
    Сравнить полученный результат с точным ответом}
    Init(1, 300, 300, 0);
    var body_x, body_y, angle, velocity, velocity_x, velocity_y,
    acceleration_x, acceleration_y, dtime: real;
    body_y := 1;
    ...
    //цикл в котором будем менять угол нач старта 
    //и будем печатать результат
    ...
    begin
        ...
        begin
            ...
            velocity_x := velocity_x + acceleration_x * dtime;
            velocity_y := velocity_y + acceleration_y * dtime;
            OutRes(body_x, body_y, col);
        //        println(velocity_x, velocity_y);
    end;
        TextOut(100, 22 * i, 'угол =' + angle.ToString + ' дальность = ' + round(body_x).ToString);
...
begin
  {Построить траекторию тела,брошенного под углом а=30 и 
    начальной скоростью 50 м/с (ЕСТЬ силы сопротивления). }
    Init(1, 700, 700, 0);
    var body_x, body_y, angle, velocity, velocity_x, velocity_y,
    acceleration_x, acceleration_y, dtime, k2, mass: real;
    body_y := 0.1;
    body_x := 0;
    velocity := 500;
    angle := 45;
    ...
    while body_y > 0 do
    begin
        acceleration_x := -k2 * velocity_x * abs(velocity_x) / mass; // сопротивление по x
        acceleration_y := -9.8 - k2 * velocity_y * abs(velocity_y) / mass; // сопротивление по y
        body_x := body_x + velocity_x * dtime;
        body_y := body_y + velocity_y * dtime;
...
begin
  {Построить траекторию тела,брошенного под углом а=30 и 
    начальной скоростью 50 м/с (ЕСТЬ силы сопротивления и ветер). }
    Init(1, 700, 700, 0);
    var body_x, body_y, angle, velocity, velocity_x, velocity_y,
    acceleration_x, acceleration_y, dtime, k2, mass, wind_x: real;
    body_y := 0.1;
    body_x := 0;
    velocity := 50;
    angle := 45;
    ...
    while body_y > 0 do
    begin
        acceleration_x := -k2 * velocity_x * abs(velocity_x) / mass + wind_x; // сопротивление по x
        acceleration_y := -9.8 - k2 * velocity_y * abs(velocity_y) / mass; // сопротивление по y
...
begin
    var numberOfTask := 328; //ReadInteger;
